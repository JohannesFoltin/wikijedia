...
func updateFolderParentFolderID(w http.ResponseWriter, r *http.Request) {
	var folderObj Folder
	id := r.PathValue("id")

	result := db.First(&folderObj, "id = ?", id)
	if result.Error != nil {
	...
	folderObj.ParentID = updatedData.ParentFolderID

	result = db.Save(&folderObj)
	if result.Error != nil {
		http.Error(w, result.Error.Error(), http.StatusInternalServerError)
		return
	}
